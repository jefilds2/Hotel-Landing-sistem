    ...
    try {
        if ($acao === 'criar_estadia' || $acao === 'editar_estadia') {
            ...
            if ($acao === 'criar_estadia') {
                ...
                redirecionar('/admin/quarto.php?id=' . $quartoId . '&estadia_id=' . $novaEstadiaId);
            }
            ...
            redirecionar('/admin/quarto.php?id=' . $quartoId . '&estadia_id=' . $estadiaIdPost);
        }
        ...
        if ($acao === 'check_out') {
            ...
            redirecionar('/admin/quarto.php?id=' . $quartoId);
        }
        ...
        redirecionar('/admin/quarto.php?id=' . $quartoId . ($estadiaIdPost > 0 ? '&estadia_id=' . $estadiaIdPost : ''));
    }
}
      ...
      <div class="lista-estadias">
        ...
        <?php foreach ($estadiasAtivas as $e): ?>
          <article class="estadia-item <?= $estadiaSelecionada && (int)$estadiaSelecionada['id'] === (int)$e['id'] ? 'ativa' : '' ?>">
            ...
            <div style="display:flex; gap:8px; align-items:center;"><span class="badge <?= e(classe_status_estadia($e['status'])) ?>"><?= e($e['status']) ?></span><a class="btn btn-neutral" href="/admin/quarto.php?id=<?= e((string)$quartoId) ?>&estadia_id=<?= e((string)$e['id']) ?>">Abrir</a></div>
          </article>
        <?php endforeach; ?>
      </div>
    </section>
